...
LinkedList l;  // Here we make the class object 

// base case 
if(!root){
    l.head=l.tail=NULL;
    return l;  // return class object
}
...
l.head=l.tail=root;
return l; // new LinkedList return 

} 

// 2.LST NOT NULL but  RST is NULL 
...
// 3: LST is NULL and RST is Exist 
else if(root->left==NULL and root->right!=NULL){
...
// 4:  LST are NOT NULL and RST are NOT NULL
else{
LinkedList left= BSTtoLL(root->left);
LinkedList right=BSTtoLL(root->right);
left.tail->right=root;
